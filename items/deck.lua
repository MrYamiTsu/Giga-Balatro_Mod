...
SMODS.Back { --DrrunnkiDaecpk
    ...
    apply = function(self, back)
        G.E_MANAGER:add_event(Event({
            func = function()
                for i, card in pairs(G.playing_cards) do
                    if pseudorandom('ok0' .. i, 1, 26) == 1 then
        ...
        G.GAME.starting_params.hands = pseudorandom('ok1' .. os.time(), 1, 7)
        G.GAME.starting_params.discards = pseudorandom('ok2' .. os.time(), 2, 8)
        G.GAME.starting_params.dollars = pseudorandom('ok3' .. os.time(), 0, 15)
        G.GAME.starting_params.joker_slots = pseudorandom('ok4' .. os.time(), 3, 6)
        G.GAME.starting_params.consumable_slots = pseudorandom('ok5' .. os.time(), 1, 4)
        G.GAME.starting_params.hand_size = pseudorandom('ok6' .. os.time(), 6, 10)
